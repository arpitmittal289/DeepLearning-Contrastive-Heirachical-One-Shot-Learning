...
# In[]:
print("PRETRAINED MODEL USING FEW SHOT LEARNING")
...
def trainEncoderForLevel(model, loss_fn, optimizer, scheduler, level, num_epochs, train_data, val_data, test_data):
    ...
    torch.save(model,"L"+str(level)+"_FEWSHOT_PRETRAINED_MODEL")

    print("Average Level " + str(level) + " Test cosine Similarity:", max(cosine_sim))
...
def plotTrainValCurve(train_loss, val_loss, level):
    ...
    plt.plot(num_batch,train_loss, label="Cross-validation score")
    plt.plot(num_batch,val_loss, label="Train Loss")
    plt.xlabel('Epochs') 
    plt.ylabel("Loss") 
