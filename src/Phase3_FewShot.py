...
def trainEncoderForLevel(model, loss_fn, optimizer, scheduler, level, num_epochs, train_data, val_data, test_data):
            ...
            if i>0 and i%batchStep == 0:
                with torch.no_grad():
                    for images, embeddings in val_data:
                        ...
                        cos_sim = metrics.pairwise.cosine_similarity(image_embeddings.flatten(start_dim=1).cpu(), embeddings.flatten(start_dim=1).cpu())
                        cosine_sim_lst.extend(cos_sim)
                #scheduler.step(metrics = val_loss)
                train_loss_per_Batch.append(np.array(train_loss_lst).mean())
                val_loss_per_Batch.append(np.array(val_loss_lst).mean())
        print('Epoch [{}/{}], Val Loss: {:.4f}, Cosine Sim: {:.4f}'
          .format(epoch+1, num_epochs, np.array(val_loss_lst).mean(), np.array(cosine_sim_lst).mean()))
    '''    
    '''
    ...
    print("Average Level " + str(level) + " Test cosine Similarity:", np.array(cosine_sim).max())
